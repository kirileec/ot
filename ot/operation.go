...
func (t *Operation) Insert(s string) *Operation {
	...
	r := []rune(s)
	if TextEncoding == TextEncodingTypeUTF16 {
...
func (t *Operation) Apply(s string) (string, error) {
	...
	if len(r) != t.BaseLen {
		return "", ErrOperationBaseLenMismatch
	}
	...
	for _, op := range t.Ops {
		if IsRetain(op) {
			...
			ss := r[i : i+op.N]
			if TextEncoding == TextEncodingTypeUTF8 {
				newStr += string(ss)
			} else {
...
func OperationTransform(a, b *Operation) (*Operation, *Operation, error) {
	if a.BaseLen != b.BaseLen {
		return nil, nil, ErrOperationBaseLenMismatch
	}
	...
	for !(opA == nil && opB == nil) {
		...
		if opA == nil || opB == nil {
			return nil, nil, ErrOperationTransformFailed
		}
...
func OperationUnmarshal(ops []interface{}) (*Operation, error) {
	...
	for _, o := range ops {
		...
		case string:
			...
			top.Insert(s)
		default:
			return nil, ErrOpeartionUnmarshalFailed
		}
	}
	return top, nil
}
